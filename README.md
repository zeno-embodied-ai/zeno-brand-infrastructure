...
当前为 **Phase 1：域名组合规划**。Phase 0 资产盘点已经完成；当前只建立 P0–P3 域名矩阵、预算门禁和三年持有策略。候选域名可注册性均为 `TBD`，不会登录注册商、购买或续费域名、修改 DNS、创建云资源或部署站点。
...
基础工程处于规划阶段。`zeno-ai.org` 是项目方确认的现有生产主域；所有候选域名可注册性均为 `TBD`，购买建议均为 **proposed（提议）**，不代表域名可得、已购买或 DNS 已配置。
