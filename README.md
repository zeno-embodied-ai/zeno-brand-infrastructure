...
当前为 **Phase 4：DNS 迁移与配置方案**。Phase 0–3 已完成；本阶段新增了 `zeno-ai.org` 的占位 DNS 模板、代理策略、验证与回滚清单，以及 Google Workspace 邮件 DNS 保护清单。当前权威 DNS、现网记录、Cloudflare 采用状态和服务 Origin 均未核验，未创建云项目、切换 nameserver、修改 DNS 或发布部署。
...
基础工程已完成 Phase 4 DNS 规划。`zeno-ai.org` 是项目方确认的现有生产主域；候选域名仍为 `TBD`，官网与文档站构建产物只存在于本地 `website/out/` 和 `documentation/build/`，不代表任何站点已经部署或 DNS 已配置。DNS 模板均为占位符，不代表现网记录或批准的 Cloudflare 迁移。
