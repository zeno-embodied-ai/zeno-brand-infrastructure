...
当前为 **Phase 2：官方网站第一版**。Phase 0 资产盘点和 Phase 1 域名组合规划已经完成；`website/` 已实现可静态导出的 Next.js 网站，但尚未创建 Cloudflare Pages/Vercel 项目、绑定域名或发布生产部署。
...
基础工程已完成 Phase 2 本地实现。`zeno-ai.org` 是项目方确认的现有生产主域；候选域名仍为 `TBD`，官网构建产物只存在于本地 `website/out/`，不代表网站已经部署或 DNS 已配置。
