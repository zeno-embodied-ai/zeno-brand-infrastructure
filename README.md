...
当前为 **Phase 0：分析与规划**。仓库只包含文档、占位目录和离线检查，不会连接生产账户、修改 DNS、创建云资源或部署站点。
...
├── docs/                              # 架构、域名矩阵和运行手册
├── website/                           # zeno-ai.org 官网工程（待选型）
├── documentation/                     # docs.zeno-ai.org 文档站工程（待选型）
├── infrastructure/
...
1. 从 [`docs/brand-architecture.md`](docs/brand-architecture.md) 了解品牌和系统边界。
2. 在 [`docs/domain-matrix.md`](docs/domain-matrix.md) 审核域名用途、负责人和发布阶段。
3. 生产实施前逐步执行 [`docs/deployment-runbook.md`](docs/deployment-runbook.md)，每个批准点均需人工确认。
...
基础工程处于规划阶段，所有域名和系统映射均为 **proposed（提议）**，不代表 DNS 或生产资源已经存在。
